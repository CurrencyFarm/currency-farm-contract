...
contract CurrencyFarmTokenCore is ERC20("Currency Farm Token", "CNA") {
    using SafeMath for uint256;
...
contract CftToken is CurrencyFarmTokenCore, Ownable {
...
contract LPToCFT is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for CftToken;
    using SafeERC20 for IERC20;
    using Address for address;

    CftToken private cft;
    ...
    // initial amount of cft
    uint256 public initReward = 720000 * 1e18;
    ...
    uint256 public devFundAmount = 175200 * 1e18;
    uint256 public devDistributeDuration = 36500 days;
    ...
    constructor(address _cft, address _lpToken) {
        cft = CftToken(_cft);
    ...
    function claim(uint amount) public shouldStarted {
        ...
        cft.safeTransfer(msg.sender, amount);
        distributeDevFund();
    ...
    function checkHalving() internal {
        if (block.timestamp >= halvingTime) {
            ...
            cft.mint(address(this), initReward);
    ...
    function startFarming() external onlyOwner {
        ...
        cft.mint(address(this), mintAmount);
    ...
    function distributeDevFund() internal {
        ...
        cft.safeTransfer(devAddr, fundAmount);
    ...
    function cftAddress() view public returns(address) {
        return address(cft);
    }

    function testMint() public onlyOwner {
        cft.mint(address(this), 1);
    }
}